...
Documentation stays at the repository root (`README.md`, `ROADMAP.md`). Core Zig code lives in `src/`: `root.zig` exports the library surface, `main.zig` drives the CLI, and `src/core/` holds shared modules (`schema.zig`, `format.zig`, `serialize.zig`, `deserialize.zig`, `target.zig`, `migrate.zig`, `changeset.zig`, `validators.zig`, `query.zig`). Add future modules under `src/<area>/` and keep any runtime helpers alongside the code they serve. House automated tests under `tests/` (mirroring the `src/` tree) so source modules remain uncluttered. Reserve `docs/` for design notes, and treat `zig-out/` as build output that never lands in Git.

## Build, Test, and Development Commands
Run `zig build` to compile the library and install the CLI into `zig-out/bin/zdl`. Use `zig build run -- <args>` for quick manual smoke checks. Execute `zig build test` for the full suite (aggregated via `tests/main.zig`), and `zig build test -Doptimize=ReleaseFast --summary all` before shipping to confirm canonical byte output. For focused runs use `zig test tests/<area>/<case>.zig`. Keep formatting clean with `zig fmt src/**/*.zig tests/**/*.zig build.zig`. House experimental utilities under `tools/` and execute them via `zig run tools/<name>.zig`.
